...
public class MetaData {
  ...
  public MetaData(String assetName, JSONObject attributes, String policyId) {
    if (assetName.length() > assetNameMaxLength) {
      throw new IllegalArgumentException(
          String.format(
              "asset_name max length (%s) exceeded: length %s",
              assetNameMaxLength, assetName.length()));
    }
    if (!attributes.has("name") || !attributes.has("image")) {
      throw new IllegalArgumentException("attributes must contain \"name\" & \"image\" keys");
    }
  ...
  /**
   * Add file to "files" : [] of Nft meta data. This method is required to be run at least once
   * before getFiles() can be used.
   */
  ...
  /**
   * Returns "files" : [] of Nft meta data, format is JSONArray of MetaDataFile JSONObjects. Will
   * only return once a file has been added via addFile method. Otherwise 'org.json.JSONException'
   * exception is thrown.
   *
